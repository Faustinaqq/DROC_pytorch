...
class BaseTrain(object):
  ...
  """
    def __init__(self, args):
        ...
        self.net_width = args.net_width  # channels
    ...
    def config(self):
        ...
        self.get_dataloader()
        # Model architecture.
        self.model = self.get_model(arch=self.net_type,
    ...
    def get_dataloader(self):
        ...
        datasets = dl.load_dataset(is_validation=self.is_validation,
    ...
    def get_model(arch='ResNet18',
        ...
        net = model.__dict__[arch](width=width,
                                   head_dims=head_dims,
    ...
    def get_scheduler(sched_type='cosine',
                      ...
                      **kwargs):
        """Gets the scheduler."""
        scheduler = CustomSchedule(step_per_epoch=step_per_epoch,
    ...
    def get_current_train_epoch(self):
        ...
        return tf.math.floordiv(self.optimizer.iterations,
                                self.num_batch).numpy()
    ...
    def train(self):
        ...
        if self.num_epoch == 0:
            ...
            for _ in range(start_epoch, self.num_epoch):
                self.train_epoch_begin()
                self.train_epoch()
                self.train_epoch_end(is_eval=False,
                                     is_save=(self.epoch %
                                              self.ckpt_epoch == 0))
    ...
    def train_begin(self):
        ...
        self.metrics.update({
            ...
            for key in self.list_of_metrics if key.startswith(('loss'))
        })
        self.metrics.update({
            key: tf.keras.metrics.Accuracy()
            for key in self.list_of_metrics if key.startswith('acc')
        })
        self.monitor = {
            'learning_rate': 0,
            'step_per_second': 0,
    ...
    def train_end(self, verbose=False):
        ...
        if verbose:
            ...
            event_dict = {
                key: []
                for key in self.metrics.keys() if not key.startswith('monitor')
            }
            ...
            event_dict = {
                key: event_dict[key][-num_epoch_to_save:]
            ...
            for key in event_dict:
                ...
                with open(os.path.join(self.json_path, key + '.json'),
                          'w') as outfile:
            ...
            logging.info(
                '----------------------------------------------------------')
            logging.info(
                'Train is done. Below are file path and basic test stats\n')
            ...
            with tf.io.gfile.GFile(
                    os.path.join(self.json_path, 'summary.json'),
                    'w') as outfile:
                json.dump(self.eval_metrics, outfile, sort_keys=True, indent=4)
        with tf.io.gfile.GFile(os.path.join(self.json_path, 'args.json'),
                               'w') as outfile:
            json.dump(self.args, outfile, indent=4, sort_keys=True)

    def train_epoch(self):
        """Called for model training per epoch."""
        time_init = time.time()
        for _ in trange(self.num_batch,
                        leave=False,
                        desc='Epoch (train) %d/%d' %
                        (self.epoch + 1, self.num_epoch)):
            self.train_step(self.train_iterator)
        self.monitor['step_per_second'] = self.num_batch / (time.time() -
                                                            time_init)
    ...
    def train_epoch_end(self, is_eval=False, is_save=False):
        ...
        if is_save:
            self.manager.save()
        if is_eval:
            self.eval_epoch(trainset=self.cls_iterator,
                            testset=self.test_iterator)
    ...
    def train_step(self, iterator):
        ...
        # 这里损失的意思应该是区分增强的方法？（只考虑前俩个？）
        def step_fn(data):
            replica_context = tf.distribute.get_replica_context()
            xo, xc = data[0], data[1]
            x = tf.concat((xo, xc), axis=0)
            y = tf.concat((tf.zeros(xo.shape[0], dtype=tf.int32),
                           tf.ones(xc.shape[0], dtype=tf.int32)),
                          axis=0)
            with tf.GradientTape() as tape:
                ...
                loss_xe = tf.divide(
                    ...
                    self.cross_replica_concat(
                        loss_xe, replica_context=replica_context).shape[0])
                loss_l2 = self.loss_l2(self.model.trainable_weights)
                loss = loss_xe + self.weight_decay * loss_l2
            grad = tape.gradient(loss, self.model.trainable_weights)
            self.optimizer.apply_gradients(
                zip(grad, self.model.trainable_weights))
            # monitor
            ...
            self.metrics['acc.train'].update_state(y, tf.argmax(logits,
                                                                axis=1))
    ...
    def squared_difference(self, a, b, do_normalization=True):
        ...
        return tf.norm(a, axis=1, keepdims=True)**2 + tf.transpose(
            tf.norm(b, axis=1, keepdims=True)**
            2) - 2. * tf.matmul(a, b, transpose_b=True)
    ...
    def eval_embed(self, trainset, testset):
        ...
        dist_embed = tf.reduce_mean(1.0 - tf.nn.top_k(sim_embed, k=1)[0],
                                    axis=1)
        ...
        for key in self.eval_metrics:
            if key.startswith('logit'):
                ...
                feats = pools.numpy()
                sim = sim_pool
                np.save("./save/{}_train_pool.npy".format(self.category),
                        feats_tr)
                np.save("./save/{}_test_pool.npy".format(self.category), feats)
                np.save("./save/{}_test_labels".format(self.category),
                        labels.numpy())
            if 'auc' in key:
                ...
                gamma = 10. / (tf.math.reduce_variance(feats_tr) *
                               feats_tr.shape[1])
                clf = OneClassSVM(kernel='rbf', gamma='scale').fit(feats_tr)
                scores = -clf.score_samples(feats)
                self.eval_metrics[key] = util_metric.roc(pr=scores, gt=labels)
                ...
                for i in range(num_iter):
                    sim_batch = sim[i * batch_size_for_kde:(i + 1) *
                                    batch_size_for_kde]
                    scores_batch = -tf.divide(
                        tf.reduce_logsumexp(2 * gamma * sim_batch, axis=1),
                        gamma)
    ...
    def extract(self, dataset):
        ...
        for _ in trange(num_batch,
            ...
            if num_aug > 1:
                ...
                probs = tf.split(probs,
                                 num_aug)  # num_aug个最初样本数（batch * probs_dim?)
                dscore = tf.math.exp(
                    tf.reduce_sum(tf.math.log(
                        tf.concat(
                            [probs[i][:, i:i + 1] for i in range(len(probs))],
                            axis=1)),
                                  axis=1))
        ...
        return (tf.nn.softmax(outputs['logits'],
                              axis=1), outputs['dscore'], outputs['embeds'],
                outputs['pools'], tf.squeeze(outputs['labels']))
    ...
    def monitor_progress(self, verbose=False):
        ...
        with self.summary_writer.as_default():
            ...
            for key, metric in self.metrics.items():
                tf.summary.scalar(key.replace('.', '/', 1),
                                  metric.result(),
                                  step=vis_step)
            tf.summary.scalar('monitor/step_per_second',
                              self.monitor['step_per_second'],
                              step=vis_step)
            tf.summary.scalar('monitor/lr',
                              self.monitor['learning_rate'],
                              step=vis_step)
            if verbose:
        ...
        logging.info(
            template.format(epoch=self.epoch + 1,
    ...
    """
        ...
        with tf.name_scope('cross_replica_concat'):
            ...
            return tf.reshape(ext_tensor,
                              [-1] + ext_tensor.shape.as_list()[2:])
