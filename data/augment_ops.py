...
class RandomShift(object):
    ...
    def __call__(self, image, is_training=True):
        if is_training:
            ...
            image = np.pad(image, [[self.pad[0]] * 2, [self.pad[1]] * 2, [0] * 2], mode='reflect')
...
class RandomCropAndResize(object):
    ...
    def __call__(self, image, is_training=True):
        if is_training:
            # crop and resize
            width = np.random.uniform(low=np.cast(image.shape[0]) * self.min_scale, high=image.shape[0]+1, size=[]) 
            ...
            #                           dtype=tf.int32)
            size = (width, np.minimum(width, image.shape[1]), image.shape[2])   # tf.minimum(width, image.shape[1]), image.shape[2])
...
class Rotate90(object):
    ...
    def __call__(self, image, is_training=True):
        ...
        return np.rot90(image, k=1) if is_training else image
...
class Rotate180(object):
    ...
    def __call__(self, image, is_training=True):
        return np.rot90(image, k=2) if is_training else image
...
class Rotate270(object):
    ...
    def __call__(self, image, is_training=True):
        return np.rot90(image, k=3) if is_training else image
...
class ColorJitter(object):
    ...
    def __call__(self, image, is_training=True):
        return transforms.ColorJitter(self.brightness, self.contrast, self.saturation, self.hue)(image) if is_training else image
...
class RandomBlur(object):
    ...
    def __call__(self, image, is_training=True):
        if np.random.uniform(0, 1) > self.prob:
            return transforms.GaussianBlur(kernel_size=image.shape[1] // 10)(image) if is_training else image
        else:
            return image
