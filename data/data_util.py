...
class BasicImageProcess():
    ...
    def image_normalize(self, image, do_mean=True, do_std=True):
        ...
        if do_mean:
            means = np.broadcast_to(channel_means, np.shape(image))
            # means = tf.broadcast_to(channel_means, tf.shape(image))
            image = image - means
        if do_std:
            stds = np.broadcast_to(channel_stds, np.shape(image))
            image = image / stds
        return image

    def preprocess_image(self, image, dtype=np.float32, aug_ops_list=None):
        """Preprocess images."""
        image = np.cast(image, dtype) / 255.0
        image = np.reshape(image, shape=np.stack(
            self.input_shape))  # may become problematic
        images = apply_augment(image, ops_list=aug_ops_list)
        return images

# return tuple : (aug_image1, ..., label, image_id)
