...
def rotate_array_deterministic(data):
  ...
  """
    return np.concatenate(
        [data] + [np.rot90(data, k=k, axes=(1, 2)) for k in range(1, 4)],
        axis=0)
...
class CIFAR(object):
    ...
    def __init__(self, root, dataset='cifar10', input_shape=(32, 32, 3)):
        ...
        train_dataset, test_dataset = self.load_data(dataset=dataset_raw,
                                                      label_mode=label_mode)
        self.trainval_data = [
            train_dataset.data, train_dataset.targets,
            np.expand_dims(np.arange(len(train_dataset)), axis=1)
        ]
        self.test_data = [
            test_dataset.data, test_dataset.targets,
            np.expand_dims(np.arange(len(test_dataset)), axis=1)
        ]
    ...
    #                 os.path.join(self.root, f'data_batch_{i}')
    #                 for i in range(1, 6)
    #             ])
    ...
    def load_dataset(self,
        ...
        if is_validation:
            ...
            test_data = [
                np.concatenate((self.trainval_data[0][idx_val_neg],
                               ...
                               axis=0),
                np.concatenate(
                    (np.zeros_like(self.trainval_data[1][idx_val_neg]),
        ...
        if distaug_type:
            ...
            if distaug_type == 'rot':
                ...
                lab_data = np.concatenate([train_data[1] for _ in range(4)],
                                          axis=0)
            elif distaug_type == 'hflip':
                aug_data = hflip_array_deterministic(train_data[0])
                lab_data = np.concatenate([train_data[1] for _ in range(2)],
                                          axis=0)
                ...
                aug_data = geotrans_array_deterministic(
                    train_data[0], distaug_type)
                lab_data = np.concatenate(
                    [train_data[1] for _ in range(distaug_type)], axis=0)
            train_data = [
                aug_data, lab_data,
                np.arange(len(aug_data))[:, None]
            ]
        ...
        train_loader_for_cls = train_set_for_cls.input_fn(
            is_training=False,
            batch_size=batch_size if aug_list_for_test is None else
            batch_size // len(aug_list_for_test),
            aug_list=test_aug,
