...
class GaussianMixture(torch.nn.Module):
    ...
    """
    def __init__(self, n_components, n_features, covariance_type="full", eps=1.e-6, init_params="kmeans", mu_init=None, var_init=None, batch_size=1000):
        """
    ...
    def _m_step(self, x, log_resp):
        ...
        B = math.ceil(x.size(0) / batch_size)
        if self.covariance_type == "full":
            eps = (torch.eye(self.n_features) * self.eps).to(x.device)
            var = torch.zeros((1, self.n_components, self.n_features, self.n_features), device=x.device)
            for i in range(B):
                ...
                var += torch.sum((x[i*batch_size: (i+1)*batch_size] - mu).unsqueeze(-1).matmul((x[i*batch_size: (i+1)*batch_size] - mu).unsqueeze(-2)) * resp[i*batch_size: (i+1)*batch_size].unsqueeze(-1), dim=0,keepdim=False)
                var = var / torch.sum(resp, dim=0, keepdim=True).unsqueeze(-1) + eps
            # print("x.size: ", x.size(), "mu.size: ", mu.size(), "var.size: ", var.size(), "resp.size: ", resp.size(), "x-mu: ", (x-mu).size())
